...
export default function PictorialStage({
    ...
    return (
        <div className="space-y-16">
            ...
            <div className="bg-white/60 backdrop-blur-xl rounded-[48px] p-8 md:p-12 border border-[#fff4e6] shadow-2xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-64 h-64 bg-pink-100/30 rounded-full blur-3xl -z-10 -translate-y-1/2 translate-x-1/2" />
                ...
                {style === 'custom' && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="max-w-2xl mx-auto space-y-4 mt-12">
                        <label className="block text-xl font-bold text-[#2d241a] font-outfit flex items-center gap-2">
                            <Stars className="text-pink-500 fill-pink-500 w-5 h-5 animate-pulse" />나만의 커스텀 스타일 제안
                        </label>
                        <textarea
                            value={customPrompt}
                            onChange={(e) => setCustomPrompt(e.target.value)}
                            placeholder="예: 전통 한복을 입은 귀여운 도령님..."
                            className="w-full h-40 px-8 py-6 rounded-[32px] border-2 border-[#fff4e6] outline-none resize-none text-lg bg-white/80 text-[#2d241a] font-medium leading-relaxed focus:border-pink-300 transition-all shadow-inner"
                        />
                    </motion.div>
                )}
            </div>

            <div className="max-w-2xl mx-auto w-full flex flex-col items-center gap-8 py-10 bg-white/60 backdrop-blur-xl rounded-[40px] border border-[#fff4e6] shadow-xl relative overflow-hidden">
                <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-pink-200 to-transparent opacity-30" />
                <div className="w-full px-12 space-y-4">
                    ...
                    <input
                        ...
                        className="w-full px-8 py-5 rounded-full border-2 border-white bg-white/80 focus:border-pink-300 outline-none text-center text-[#2d241a] font-medium transition-all shadow-inner"
                    />
                </div>
                <div className="flex flex-row gap-4 w-full px-12">
                    <button onClick={onReset} className="flex-1 px-4 py-5 rounded-[28px] font-bold text-[#8b7355] border-2 border-[#fff4e6] hover:bg-white transition-all font-outfit bg-white/50 h-[72px] text-sm active:scale-95 shadow-sm">다시 업로드</button>
                    <button
                        onClick={onSubmit}
                        disabled={loading || !style || (style === 'custom' && !customPrompt.trim())}
                        className="flex-[2] relative group/btn px-6 py-5 rounded-[28px] font-bold bg-pink-500 hover:bg-pink-400 text-white transition-all disabled:opacity-50 shadow-2xl shadow-pink-200 text-lg h-[72px] flex items-center justify-center gap-3 overflow-hidden active:scale-[0.98]"
                    >
                ...
                <label className="flex items-center gap-3 cursor-pointer group py-3 px-8 rounded-full hover:bg-white/60 transition-all border border-transparent hover:border-pink-100/50">
